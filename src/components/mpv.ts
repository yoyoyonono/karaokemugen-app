...
export class Players {
	...
	async toggleAudioOnlyExperience(): Promise<boolean> {
		try {
			playerState.currentVideoTrack === 1 ? 2 : 1;
			const showVideo = !playerState.modifiers || (playerState.modifiers && playerState.modifiers.Blind === '');
			...
			emitPlayerState();
			return playerState.currentVideoTrack === 2;
		} catch (err) {
